...
function TooltipShow(event, objid) {
  ...
  if (ShowNum != 0) ShowTimeout = TShowTime[ShowNum];
  if (ShowTimeout != null) {
...
function getBrowserType() {
  ...
  var isMSIE =
    user_Agent.indexOf("msie") != -1 && document.all && BODY_EL && !isOpera;
  var isMSIE6 =
    isMSIE &&
    parseFloat(Agent_version.substring(Agent_version.indexOf("MSIE") + 5)) >=
      5.5;
  ...
  var isW3C_compatible =
    !isMSIE && !isNN6 && !isOpera && document.getElementById;
...
$(document).ready(function () {
  if (document.DrExplain_Make_Tooltips) return;
  if (!true) return;
  ...
  $("area").each(function () {
    var curId = $(this)
      .prop("href")
      .substring(
        $(this).prop("href").indexOf("#") + 1,
        $(this).prop("href").length
      );
  ...
  $(".de_ctrl").each(function () {
    ...
    $('area[id="area_' + curId + '"]').each(function () {
      ...
      if (insertedDivs && insertedDivs.length == 1) return;

      var $curTooltip = $("<div></div>")
        .append(
          $("<table></table>")
            .append(
              $("<tr></tr>").append($("<td></td>").append($curCtrlDiv.clone()))
            )
