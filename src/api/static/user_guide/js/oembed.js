...
(function ($) {
  $.fn.oembed = function (url, options, embedAction) {
    ...
    return this.each(function () {
      var container = $(this),
        resourceURL =
          url && (!url.indexOf("http://") || !url.indexOf("https://"))
            ? url
            : container.attr("href"),
        provider;
            ...
            var ajaxopts = $.extend(
              {
                ...
                success: function (data) {
                  ...
                  if (provider !== null) {
                    provider.params =
                      getNormalizedParams(settings[provider.name]) || {};
  ...
  function getRequestUrl(provider, externalUrl) {
    ...
    if (
      provider.maxWidth !== null &&
      (typeof provider.params.maxwidth === "undefined" ||
        provider.params.maxwidth === null)
    ) {
      provider.params.maxwidth = provider.maxWidth;
    }

    if (
      provider.maxHeight !== null &&
      (typeof provider.params.maxheight === "undefined" ||
        provider.params.maxheight === null)
    ) {
  ...
  function embedCode(container, externalUrl, embedProvider) {
    if (
      $("#jqoembeddata").data(externalUrl) != undefined &&
      embedProvider.embedtag.tag != "iframe"
    ) {
      var oembedData = { code: $("#jqoembeddata").data(externalUrl) };
      success(oembedData, externalUrl, container);
    } else if (embedProvider.yql) {
      var from = embedProvider.yql.from || "htmlstring";
      var url = embedProvider.yql.url
        ? embedProvider.yql.url(externalUrl)
        : externalUrl;
      var query =
        "SELECT * FROM " +
        from +
      ...
      var ajaxopts = $.extend(
        {
          ...
          success: function (data) {
            ...
            if (
              embedProvider.yql.xpath &&
              embedProvider.yql.xpath == "//meta|//title|//link"
            ) {
              var meta = {};
              ...
              for (var i = 0, l = data.query.results.meta.length; i < l; i++) {
                var name =
                  data.query.results.meta[i].name ||
              ...
              if (
                !meta.hasOwnProperty("title") ||
                !meta.hasOwnProperty("og:title")
              ) {
              ...
              if (
                !meta.hasOwnProperty("og:image") &&
                data.query.results.hasOwnProperty("link")
              ) {
                for (
                  var i = 0, l = data.query.results.link.length;
                  i < l;
                  i++
                ) {
      ...
      if (embedProvider.embedtag.tag !== "") {
        ...
        var src = externalUrl.replace(
          embedProvider.templateRegex,
        ...
        var code = $("<" + tag + "/>")
          ...
          .attr(
            "allowfullscreen",
            embedProvider.embedtag.allowfullscreen || "true"
          )
          .attr(
            "allowscriptaccess",
            embedProvider.embedtag.allowfullscreen || "always"
          )
        ...
        if (tag == "embed")
          code
            .attr(
              "type",
              embedProvider.embedtag.type || "application/x-shockwave-flash"
            )
            .attr(
              "flashvars",
              externalUrl.replace(embedProvider.templateRegex, flashvars)
            );
        if (tag == "iframe")
          code
        ...
        //Add APIkey if true
        if (embedProvider.apikey)
          embedProvider.apiendpoint = embedProvider.apiendpoint.replace(
            "_APIKEY_",
            settings.apikeys[embedProvider.name]
          );
        ajaxopts = $.extend(
          {
        ...
        var oembedData = {
          code: externalUrl.replace(
            embedProvider.templateRegex,
            embedProvider.template
          ),
        };
        success(oembedData, externalUrl, container);
      }
        ...
        ajaxopts = $.extend(
          {
            ...
            success: function (data) {
              ...
              switch (oembedData.type) {
                ...
                case "photo":
                  oembedData.code = $.fn.oembed.getPhotoCode(
                    externalUrl,
                    oembedData
                  );
                  break;
                case "video":
                case "rich":
                  oembedData.code = $.fn.oembed.getRichCode(
                    externalUrl,
                    oembedData
                  );
                  break;
                default:
                  oembedData.code = $.fn.oembed.getGenericCode(
                    externalUrl,
                    oembedData
                  );
                  break;
  ...
  $.fn.oembed.getOEmbedProvider = function (url) {
    for (var i = 0; i < $.fn.oembed.providers.length; i++) {
      for (
        var j = 0, l = $.fn.oembed.providers[i].urlschemes.length;
        j < l;
        j++
      ) {
  ...
  $.fn.oembed.OEmbedProvider = function (
    name,
    type,
    urlschemesarray,
    apiendpoint,
    ...
    if (extraSettings.useYQL) {
      if (extraSettings.useYQL == "xml") {
        ...
        extraSettings.yql = {
          ...
          datareturn: function (results) {
            if (
              results.json.type != "video" &&
              (results.json.url || results.json.thumbnail_url)
            ) {
              return (
                '<img src="' +
                (results.json.url || results.json.thumbnail_url) +
  ...
  $.fn.oembed.providers = [
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "video",
      ["metacafe\\.com/watch/.+"],
      "http://www.metacafe.com/fplayer/$1/$2.swf",
      {
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "video",
      ["twitvid\\.com/.+"],
      "http://www.twitvid.com/embed.php?guid=$1&autoplay=0",
      {
    ...
    new $.fn.oembed.OEmbedProvider(
      "aniboom",
      "video",
      ["aniboom\\.com/animation-video/.+"],
      "http://api.aniboom.com/e/$1",
      {
        templateRegex: /.*animation-video\/(\d+).*/,
        embedtag: { width: 594, height: 334 },
      }
    ),
    ...
    new $.fn.oembed.OEmbedProvider(
      "vzaar",
      "video",
      ["vzaar\\.com/videos/.+", "vzaar.tv/.+"],
      "http://view.vzaar.com/$1/player?",
      {
        templateRegex: /.*\/(\d+).*/,
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "video",
      ["snotr\\.com/video/.+"],
      "http://www.snotr.com/embed/$1",
      {
        templateRegex: /.*\/(\d+).*/,
        embedtag: { tag: "iframe", width: 400, height: 330, nocache: 1 },
      }
    ),
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "video",
      ["embedr\\.com/playlist/.+"],
      "http://embedr.com/swf/slider/$1/425/520/default/false/std?",
      {
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "http://api.minoto-video.com/services/oembed.json",
      { useYQL: "json" }
    ),
    new $.fn.oembed.OEmbedProvider(
      "animoto",
    ...
    new $.fn.oembed.OEmbedProvider(
      "videojug",
      "video",
      ["videojug\\.com/(film|payer|interview).*"],
    ...
    new $.fn.oembed.OEmbedProvider(
      "clikthrough",
      "video",
      ["clikthrough\\.com/theater/video/\\d+"],
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "rich",
      ["official.fm/.+"],
      "http://official.fm/services/oembed",
      { useYQL: "json" }
    ),
    new $.fn.oembed.OEmbedProvider(
      "chirbit",
    ...
    new $.fn.oembed.OEmbedProvider(
      "deviantart",
      "photo",
      ["deviantart.com/.+", "fav.me/.+", "deviantart.com/.+"],
    ...
    new $.fn.oembed.OEmbedProvider(
      "mobypicture",
      "photo",
      ["mobypicture.com/user/.+/view/.+", "moby.to/.+"],
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "photo",
      ["chart\\.ly/[a-z0-9]{6,8}"],
      "http://chart.ly/uploads/large_$1.png",
      {
        templateRegex: /.*ly\/([^\/]+).*/,
        embedtag: { tag: "img" },
        nocache: 1,
    ...
    new $.fn.oembed.OEmbedProvider("gravtar", "photo", ["mailto:.+"], null, {
      ...
      template: function (wm, email) {
        return (
          ...
          email.md5() +
          '.jpg" alt="on Gravtar" class="jqoaImg">'
        );
      },
    }),
      ...
      null,
      {
        ...
        template: function (wm, tourid) {
          ...
          return (
            "<div id='" +
            tourid +
            "' class='tourwrist-tour-embed direct'></div> <script type='text/javascript' src='http://tourwrist.com/tour_embed.js'></script>"
          );
        },
      }
    ),
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "http://$1.wikipedia.org/w/api.php?action=parse&page=$2&format=json&section=0&callback=?",
      {
        ...
        templateData: function (data) {
          ...
          return (
            '<div id="content"><h3><a class="nav-link" href="http://en.wikipedia.org/wiki/' +
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "http://www.imdbapi.com/?i=$1&callback=?",
      {
        ...
        templateData: function (data) {
          ...
          return (
            '<div id="content"><h3><a class="nav-link" href="http://imdb.com/title/' +
            data.ID +
            '/">' +
            data.Title +
            "</a> (" +
            ...
            ")</h3><p>Starring: " +
            data.Actors +
            '</p><div id="photo-wrap" style="margin: auto;width:600px;height:450px;"><img class="photo" id="photo-display" src="' +
            data.Poster +
            '" alt="' +
            data.Title +
            '"></div>  <div id="view-photo-caption">' +
            data.Plot +
            "</div></div>"
          );
        },
      ...
      "http://ljpic.seacrow.com/json/$2$4?jsonp=?",
      {
        ...
        templateData: function (data) {
          ...
          return (
            ...
            data.username +
            '.livejournal.com/profile"><img src="http://www.livejournal.com/img/userinfo.gif" alt="[info]" width="17" height="17" /></a><a href="http://' +
            data.username +
            '.livejournal.com/">' +
            data.username +
            "</a></span><br />" +
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "rich",
      ["jsfiddle.net/[^/]+/?"],
      "http://jsfiddle.net/$1/embedded/result,js,resources,html,css/?",
      {
        templateRegex: /.*net\/([^\/]+).*/,
    ...
    new $.fn.oembed.OEmbedProvider(
      "timetoast",
      "rich",
      ["timetoast.com/timelines/[0-9]+"],
      "http://www.timetoast.com/flash/TimelineViewer.swf?passedTimelines=$1",
      {
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "https://api.github.com/repos/$1/$2?callback=?",
      {
        ...
        templateData: function (data) {
          ...
          return (
            '<div class="oembedall-githubrepos"><ul class="oembedall-repo-stats"><li>' +
            data.data.language +
            '</li><li class="oembedall-watchers"><a title="Watchers" href="' +
            data.data.html_url +
            '/watchers">&#x25c9; ' +
            data.data.watchers +
            "</a></li>" +
            '<li class="oembedall-forks"><a title="Forks" href="' +
            data.data.html_url +
            '/network">&#x0265; ' +
            data.data.forks +
            '</a></li></ul><h3><a href="' +
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "https://graph.facebook.com/$2$3/?callback=?",
      {
        templateRegex: /.*facebook.com\/(people\/[^\/]+\/(\d+).*|([^\/]+$))/,
        templateData: function (data) {
          if (!data.id) return false;
          var out =
            '<div class="oembedall-facebook1"><div class="oembedall-facebook2"><a href="http://www.facebook.com/">facebook</a> ';
          if (data.from)
            out +=
            ...
            out +=
              ...
              "</a>";
          out +=
            '</div><div class="oembedall-facebookBody"><div class="contents">';
          if (data.picture)
            out +=
              '<a href="' +
              data.link +
        ...
        templateData: function (data) {
          ...
          for (i in q.tags)
            out +=
              ...
              "</a>";
          out +=
            '</div><div class="oembedall-fr"><div class="oembedall-user-info"><div class="oembedall-user-gravatar32"><a href="http://stackoverflow.com/users/' +
            q.owner.user_id +
            "/" +
            ...
            q.owner.email_hash +
            '?s=32&amp;d=identicon&amp;r=PG"></a></div><div class="oembedall-user-details">' +
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "rich",
      ["gigapan\\.org/[-.\\w@]+/\\d+"],
      "http://gigapan.org/gigapans/$1/options/nosnapshots/iframe/flash.html",
      {
        templateRegex: /.*\/(\d+)\/?.*/,
    ...
    new $.fn.oembed.OEmbedProvider(
      ...
      "rich",
      ["scribd\\.com/.+"],
      "http://www.scribd.com/embeds/$1/content?start_page=1&view_mode=list",
      {
        templateRegex: /.*doc\/([^\/]+).*/,
      ...
      null,
      {
        yql: {
          ...
          datareturn: function (results) {
            ...
            return (
              '<div class="oembedall-lanyard">' + results.result + "</div>"
            );
          },
        },
      }
      ...
      null,
      {
        yql: {
          ...
          datareturn: function (results) {
            if (!results.result) return false;
            return (
              '<pre style="background-color:000;">' + results.result + "</div>"
            );
          },
        },
      }
        ...
        datareturn: function (results) {
          ...
          if (results["og:video"]) {
            ...
            embed
              .attr(
                "type",
                results["og:video:type"] || "application/x-shockwave-flash"
              )
              .css("max-height", settings.maxHeight || "auto")
              .css("max-width", settings.maxWidth || "auto");
