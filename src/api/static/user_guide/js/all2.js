...
DR_EXPLAIN.dataManager = (function () {
  ...
  function drex_node(ndx) {
    ...
    this.children = function () {
      ...
      for (
        var i = drex_node_child_start[this.node_index];
        i < drex_node_child_end[this.node_index];
        i++
      )
    ...
    this.childrenCount = function () {
      if (
        drex_node_child_start[this.node_index] >=
        drex_node_child_end[this.node_index]
      )
        return 0;
      return (
        drex_node_child_end[this.node_index] -
    ...
    this.parent = function () {
      if (drex_node_parent[this.node_index] == -1) return null;
      return new drex_node(drex_node_parent[this.node_index]);
    };
    this.isActive = function () {
      if (!drex.active_node) return false;
  ...
  function drex_keyword(ndx) {
    ...
    this.children = function () {
      var result = new Array();
      for (
        var i = drex_keyword_child_start[this.keyword_index];
        i < drex_keyword_child_end[this.keyword_index];
        i++
      )
    ...
    this.childrenCount = function () {
      if (
        drex_keyword_child_start[this.keyword_index] >=
        drex_keyword_child_end[this.keyword_index]
      )
        return 0;
      return (
        drex_keyword_child_start[this.keyword_index] -
    ...
    this.parent = function () {
      if (drex_keyword_parent[this.keyword_index] == -1) return null;
      return new drex_keyword(drex_keyword_parent[this.keyword_index]);
    };
    this.nodes = function () {
      var result = new Array();
      for (
        var i = drex_keyword_nodes_start[this.keyword_index];
        i < drex_keyword_nodes_end[this.keyword_index];
        i++
      )
  ...
  var initIndex = function () {
    ...
    for (var i in drex_keyword_names) {
      for (
        var j = drex_keyword_child_start[i];
        j < drex_keyword_child_end[i];
        j++
      ) {
    ...
    for (var i = 0; i < drex.nodes_count; i++)
      for (
        var j = drex_node_keywords_start[i];
        j < drex_node_keywords_end[i];
        j++
      ) {
    ...
    drex_node.prototype.keywords = function () {
      var result = new Array();
      for (
        var i = drex_node_keywords_start[this.node_index];
        i < drex_node_keywords_end[this.node_index];
        i++
      )
  ...
  var API = {
    ...
    getErrorInRemoteSearch: function (code) {
      return data_search.DREXPLAIN_ERROR_REMOTE_SEARCH.replace(
        "{0}",
        code.toString()
      );
    },
    ...
    getRootNodesArray: function () {
      if (!data_menu.DREX_HAS_ROOT_NODE) return drex.root_node().children();
      return [drex.root_node()];
    },
...
DR_EXPLAIN.dom = {
  ...
  init: function () {
    ...
    this.$articleGeneratorCopyright = this.$article.find(
    ...
    this.tabs.menu.$wrapperItemInner = this.tabs.menu.$wrapperItem.children(
    ...
    this.tabs.index.$wrapperItemInner = this.tabs.index.$wrapperItem.children(
      ".b-tabs__wrapperItemInner"
    );
    this.tabs.index.$tree = this.tabs.index.$wrapperItemInner.children(
      ".b-tree"
    );
    ...
    this.tabs.search.$wrapperItemInner = this.tabs.search.$wrapperItem.children(
      ".b-tabs__wrapperItemInner"
    );
    this.tabs.search.$tree = this.tabs.search.$wrapperItemInner.children(
      ".b-tree"
    );

    // todo: remove or refactor

    this.$tabWrapperItemArr = this.tabs.menu.$wrapperItem
      .add(this.tabs.index.$wrapperItem)
      .add(this.tabs.search.$wrapperItem);
    this.$tabWrapperItemInnerArr = this.tabs.menu.$wrapperItemInner
      .add(this.tabs.index.$wrapperItemInner)
      .add(this.tabs.search.$wrapperItemInner);
  ...
  isIeLessThan9: function () {
    if (this._isIeLessThan9 === null) {
      if (
        this.$html.hasClass("ie6") ||
        this.$html.hasClass("ie7") ||
        this.$html.hasClass("ie8")
      ) {
  ...
  isTabVisible: function (tab) {
    if (
      ...
      !this.$workZoneSideNav.hasClass("topmost")
    ) {
      return false;
    }
  ...
  isPageHeaderVisible: function () {
    return !(
      this.$pageContent.hasClass(this.PAGE_CONTENT_HEADER__HIDDEN_CLASS) ||
      _.contains(["xs"], this.getPageLayoutType())
    );
  },
  ...
  isPageFooterVisible: function () {
    return !(
      this.$pageContent.hasClass(this.PAGE_CONTENT_FOOTER__HIDDEN_CLASS) ||
      _.contains(["xs"], this.getPageLayoutType())
    );
  },
  ...
  isPageLeftVisible: function () {
    return !(
      this.$pageContent.hasClass(this.PAGE_CONTENT_LEFT__HIDDEN_CLASS) ||
      _.contains(["xs"], this.getPageLayoutType())
    );
  },
  ...
  isPageRightVisible: function () {
    return !(
      this.$pageContent.hasClass(this.PAGE_CONTENT_RIGHT__HIDDEN_CLASS) ||
      _.contains(["xs"], this.getPageLayoutType())
    );
  },
...
DR_EXPLAIN.wordSplitter = (function () {
  ...
  Ranges.prototype.checkContains = function (val) {
    ...
    if (nBegin <= val && val < nEnd) return true;
    return false;
  };
  Ranges.prototype.upperBound = function (val) {
    ...
    while (lo < hi) {
      ...
      if (this.starts[mid] <= val) lo = mid;
      else hi = mid - 1;
    }
    if (this.starts[lo] <= val) return lo + 1;
  ...
  function isPunctuationSymbol(codePoint) {
    ...
    if (stPunctuation.checkContains(codePoint)) return true;
    return false;
  }
  ...
  function isWordSymbol(codePoint) {
    ...
    if (stWordSymbols.checkContains(codePoint)) return true;
    return false;
  }
  function toCodePoints(str) {
    ...
    for (var lengthProcessed = 0; lengthProcessed < str.length; ) {
      ...
      chars.push(ch);
      if (ch > 0xffff) lengthProcessed += 2;
      else ++lengthProcessed;
    }
    return chars;
  }
  ...
  function splitString(str) {
    ...
    for (var i = 0; i <= chars.length; ++i) {
      if (
        i != chars.length &&
        isWordSymbol(chars[i]) &&
        !isPunctuationSymbol(chars[i])
      ) {
        word.push(chars[i]);
      } else {
        if (word.length != 0) {
...
DR_EXPLAIN.searchEngine = (function () {
  ...
  function intersect(setA, setB) {
    ...
    return filter(tags, function (value) {
      return value == "AB";
    });
  }
  ...
  function SearchInFile() {
    ...
    if (iStringToSearch == 0)
      //this is first result - adding all curResults to SearchResults
      SearchResults = SearchResults.concat(curResults);
    else SearchResults = intersect(SearchResults, curResults);
  ...
  function AttachFilesToStrings() {
    for (var i = 0; i < StringsForSearch.length; i++) {
      ...
      switch (strcmp(st, IndexOfFiles[j].first)) {
        case -1: {
          if (IndexOfFiles[j].first.indexOf(st) == 0)
            //st = 'skin', IndexOfFiles[j].first = 'skinner'
            bFound = true;
          break;
        }
        case 0: {
        ...
        case 1: {
          //st = 'skinner', IndexOfFiles[j].first = 'skin'
          switch (strcmp(st, IndexOfFiles[j].last)) {
            ...
            case 1: {
              if (st.indexOf(IndexOfFiles[j].last) == 0)
                //st = 'skinner', IndexOfFiles[j].last = 'skin'
                bFound = true;
              break;
            }
          }
      ...
      if (request.readyState === 4) {
        if (
          ...
          try {
            ...
            else DR_EXPLAIN.searchManager.showLocalSearchError();
            return;
          }
  ...
  function searchmain(str) {
    ...
    for (var i = 0; i < strs.length; ++i)
      if (!isEmpty(strs[i])) StringsForSearch.push(strs[i]);
...
DR_EXPLAIN.searchManager = (function () {
  var _class = {
    ...
    doSetSearchInProgressText: function () {
      this.dom.$searchProgress.prop(
        "alt",
        this.dataManager.getSearchTextInProgress()
      );
    },
    ...
    runInputPlaceholders: function () {
      var searchInTabPlaceholder = new InputPlaceholder(
        this.dom.$tabSearchInput,
        this.dom.$tabSearchInputLabel,
        true
      );
      searchInTabPlaceholder.run();

      var searchInWorkZonePlaceholder = new InputPlaceholder(
        this.dom.$workZoneSearchInput,
        this.dom.$workZoneSearchInputLabel,
        true
      );
    ...
    runInputSync: function () {
      ...
      var $inputArr = this.dom.$tabSearchInput.add(
        this.dom.$workZoneSearchInput
      );
    ...
    doSetDom: function () {
      ...
      this.elementsArr = [
        {
          $input: this.dom.$tabSearchInput,
          $submit: this.dom.$tabSearchSubmit,
        },
        {
          $input: this.dom.$workZoneSearchInput,
          $submit: this.dom.$workZoneSearchSubmit,
        },
      ];
    },
    ...
    doSetUrlEncoder: function () {
      ...
      this.urlEncoder.addSaveFunc(function () {
    ...
    doSetQueryStringByUrlEncoder: function () {
      if (this.urlEncoder !== null) {
        var queryString = this.urlEncoder.getValueByKey(
          this.urlEncoder.KEY_NAME__SEARCH_QUERY
        );
        if (queryString !== null) {
    ...
    onClick: function (elem) {
      var s = this.searchEngine
        .trim(document.getElementById(elem.$input.prop("id")).value)
        .replace(/\r/g, "")
        .replace(/\n/g, "")
        .replace(/\t/g, " ")
        .replace(/\u00A0/g, "");
    ...
    performSearch: function (s) {
      ...
      else this.searchEngine.doSearch(s);
    },
    ...
    searchComplete: function () {
      ...
      if (searchResultsArr.length > 0) {
        ...
        var $msg = $(
          '<div class="b-tree__searchResultText">' +
            this.dataManager.getSearchTextNoResults() +
            "</div>"
        );
    ...
    showSearchTextEmptyString: function () {
      ...
      var $msg = $(
        '<div class="b-tree__searchResultText">' +
          this.dataManager.getSearchTextEmptyString() +
          "</div>"
      );
      this.dom.tabs.search.$tree.html($msg);
      $(document).trigger("searchCompleteBuildTree");
    ...
    showLocalSearchError: function () {
      ...
      var $msg = $(
        '<div class="b-tree__searchResultText">' +
          this.dataManager.getErrorInLocalSearch() +
          "</div>"
      );
      this.dom.tabs.search.$tree.html($msg);
      $(document).trigger("searchCompleteBuildTree");
    ...
    showRemoteSearchError: function (code) {
      ...
      var $msg = $(
        '<div class="b-tree__searchResultText">' +
          this.dataManager.getErrorInRemoteSearch(code) +
          "</div>"
      );
      this.dom.tabs.search.$tree.html($msg);
      $(document).trigger("searchCompleteBuildTree");
    ...
    isSearchTabSelectedOnStart: function () {
      var $navTree_search = this.dom.tabs.search.$wrapperItem;
      if (
        $navTree_search.index() ===
        parseInt(
          this.urlEncoder.getValueByKey(this.urlEncoder.KEY_NAME__TAB_INDEX)
        )
      ) {
        return true;
      } else {
  ...
  var _class = {
    ...
    doAddTabs: function () {
      if (this.DREX_SHOW_MENU)
        this.addTab(
          this.dom.tabs.menu.$selectorItem,
          this.dom.tabs.menu.$wrapperItem,
          this.dom.tabs.menu.$wrapperItemInner
        );
      if (this.DREX_SHOW_INDEX)
        this.addTab(
          this.dom.tabs.index.$selectorItem,
          this.dom.tabs.index.$wrapperItem,
          this.dom.tabs.index.$wrapperItemInner
        );
      if (this.DREX_SHOW_SEARCH)
        this.addTab(
          this.dom.tabs.search.$selectorItem,
          this.dom.tabs.search.$wrapperItem,
          this.dom.tabs.search.$wrapperItemInner
        );
    },
    ...
    doSetScrollTopByUrlEncoder: function () {
      if (this.urlEncoder !== null) {
        var scrollTop = this.urlEncoder.getValueByKey(
          this.urlEncoder.KEY_NAME__MENU_SCROLL_TOP
        );
        if (scrollTop !== null) {
          var selectedTab = this.getSelectedTab();
          ...
          var $selectedTreeElem = this.dom.tabs.menu.$tree.find(
            ".m-tree__itemContent__selected"
          );
          if ($selectedTreeElem.length > 0) {
            var selectedTreeElemTop =
              $selectedTreeElem.offset().top -
              this.dom.$tabWrapperItems.offset().top -
              $selectedTreeElem.height() / 2;
            this.dom.tabs.menu.$wrapperItemInner
              .scrollTop(selectedTreeElemTop)
              .trigger("scroll");
          }
        }
      }
    ...
    doSetTabIndexByUrlEncoder: function () {
      if (this.urlEncoder !== null) {
        var tabIndex = this.urlEncoder.getValueByKey(
          this.urlEncoder.KEY_NAME__TAB_INDEX
        );
        if (tabIndex !== null) {
    ...
    doBindEventHover: function () {
      ...
      $selectorItems.on("mouseenter", ".b-tabs__selectorItem", function () {
        if ($(this).hasClass("m-tabs__selectorItem__selected")) return false;
    ...
    getSelectedTabIndex: function () {
      ...
      if (this.tabArr.length == 0) return -1;
      return 0;
    },

    getSelectedTab: function () {
      var selectedTabIndex = this.getSelectedTabIndex();
      if (selectedTabIndex == -1) return null;
...
DR_EXPLAIN.urlEncoder = (function () {
  var _class = {
    ...
    doBindOpenNextPageWithEncodedStringToLinks: function () {
      var that = this;
      this.dom.$pageContent.on(
        "click",
        "a.local_link, a.b-breadCrumbs__link, a.b-controlButtons__link, a.b-tree__itemLink",
        function (e) {
          that.openNextPage(
            $(this).prop("href"),
            $(this).attr("href"),
    ...
    doBindOpenNextPageWithEncodedStringToLinksInKeywordContextMenu: function () {
      var that = this;
      this.dom.$keywordContextMenu.on(
        "click",
        "a.b-tree__itemLink",
        function (e) {
          that.openNextPage(
    ...
    openNextPage: function (nextpage, nextPageHref, atNewPage) {
      ...
      if (
        nextPageHref &&
        nextPageHref.length > 0 &&
        nextPageHref.charAt(0) == "#"
      ) {
        targetUrl = nextPageHref;
        nextpage = nextPageHref;
      }
      if ($("html").hasClass("ie")) targetUrl = this.utils.escapeXml(nextpage);
      var anch = "";
      var anchPos = -1;
        ...
        if ($img.length > 0) {
          ...
          if (isFinite(horizCoef) && isFinite(vertCoef)) {
            $map.find("area").each(function () {
              ...
              var originalCoordLeft = parseInt(
                $area.attr("data-coord-left"),
                10
              );
              var originalCoordTop = parseInt($area.attr("data-coord-top"), 10);
              var originalCoordRight = parseInt(
                $area.attr("data-coord-right"),
                10
              );
              var originalCoordBottom = parseInt(
                $area.attr("data-coord-bottom"),
                10
              );
  ...
  var _class = {
    ...
    doSetCurrNavWidth: function () {
      ...
      if (this._urlEncoder !== null) {
        currNavWidth = this._urlEncoder.getValueByKey(
          this._urlEncoder.KEY_NAME__MENU_WIDTH
        );
    ...
    getTabWidth: function (tabId) {
      ...
      if (curTab) return $("#" + tabId).outerWidth(true);
      return 0;
    },
    ...
    doSetArticleMinWidth: function () {
      var minWidth = parseInt(
        this.dom.$workZoneSideArticle.css("minWidth"),
        10
      );
      if ($.isNumeric(minWidth) && minWidth !== 0) {
    ...
    recalculateNavMaxWidth: function () {
      ...
      var isUserLeftSideExistsAndVisible =
        this.dom.$pageContentLeft.length > 0 && this.dom.isPageLeftVisible();
      var isUserRightSideExistsAndVisible =
        this.dom.$pageContentRight.length > 0 && this.dom.isPageRightVisible();
      ...
      if (this.dom.$headerSide__nav.length) {
        ...
        var headerSide__nav_paddings =
          this.dom.$headerSide__nav.outerWidth() -
          this.dom.$headerSide__nav.width();
        var maxBreadCrumbWidth = 0;
        this.dom.$headerSide__nav__breadCrumbs
          .find("li")
          .each(function (index) {
            maxBreadCrumbWidth = Math.max(
              maxBreadCrumbWidth,
              $(this).outerWidth()
            );
          });
        ...
        var articlePaddings =
          this.dom.$article.outerWidth() - this.dom.$article.width();
        var workZoneSideArticlePaddings =
          this.dom.$workZoneSideArticle.outerWidth() -
          this.dom.$workZoneSideArticle.width();
        var workZoneSideArticleContentPaddings =
          this.dom.$workZoneSideArticleContent.outerWidth() -
          this.dom.$workZoneSideArticleContent.width();
        var workZoneNavPaddings =
          this.dom.$workZoneSideNav.outerWidth() -
          this.dom.$workZoneSideNav.width();

        minArticleWidth = Math.max(
          minArticleWidth,
          headerSide__buttons_width +
            headerSide__nav__breadCrumbs_width +
            headerSide__nav_paddings +
            articlePaddings +
            workZoneSideArticlePaddings +
            workZoneSideArticleContentPaddings +
            workZoneNavPaddings
        );
      }
      ...
      if (!maxContentAreaWidth || maxContentAreaWidth.length < 2) {
        ...
        if (maxContentAreaWidth.charAt(maxContentAreaWidth.length - 1) == "%") {
          maxContentAreaWidth =
            (parseInt(maxContentAreaWidth, 10) * windowWidth) / 100;
        } else {
    ...
    ensureModeSizer: function () {
      if (
        this.dataManager.fitHeightToWindow() &&
        !is_touch_device() &&
        this.dom.getPageLayoutType() != "xs"
      ) {
    ...
    setNewNavWidth: function (newNavWidth) {
      ...
      var longestTreeItemWidthWithPaddings = this
        .visibleTabLongestTreeItemWidth;
      var selectedTab = this.dom.getVisibleTab();
      var tabId = null;
      if (selectedTab) tabId = selectedTab.$wrapperItem.prop("id");
      ...
      finalTreeWidth += dx;
      this.dom.$hasHorizontalScrollbar[tabId] =
        finalTreeWidth < longestTreeItemWidthWithPaddings;
...
DR_EXPLAIN.workZoneSizer_FrameMode = (function () {
  var _class = {
    ...
    doBindKeywordContextEvents: function () {
      ...
      this.dom.$keywordContextMenu.on(
        "show.contextMenu",
        function (e, $elemInTree) {
          that.dom.$keywordContextMenu_elemInTree = $elemInTree;
          that.recalculate();
        }
      );

      this.dom.tabs.index.$wrapperItemInner.on(
        "scroll.contextMenu",
        function () {
    ...
    disableRecalculatingEvents: function () {
      ...
      this.dom.$tabWrapperItemInnerArr.css({
        height: "auto",
        overflow: "auto",
      });
    ...
    recalculateIndexContextMenu: function () {
      ...
      var bottomSpaceHeight =
        workZoneHeight - topSpaceHeight - elemInTreeHeight;

      var contextMenuNewTop = 0;

      var contextMenuNewLeft = elemInTreePos.left;
      var contextMenuNewRight =
        $(window).width() - (elemInTreePos.left + $elemInTree.outerWidth());

      var contextMenuNewWidth = "auto";
      var contextMenuNewHeight = "auto";
      var contextMenuNewOverflow = "hidden";

      var isContextMenuBiggerThenBottomSpace =
        contextMenuHeight > bottomSpaceHeight;
      ...
      var isContextMenuHeightInContainer =
        !isContextMenuBiggerThenBottomSpace && !isContextMenuBiggerThenTopSpace;
    ...
    recalculateWorkZone: function () {
      var newWorkZoneHeight =
        $(window).height() - this.getHeaderHeight() - this.getFooterHeight();
    ...
    recalculateArticle: function () {
      ...
      var newArticleInnerWrapperHeight =
        ...
        this.getFooterHeight() -
        0;
      var newArticleWrapperHeight =
        newArticleHeight -
        this.dom.$articleHeader.outerHeight(true) -
        this.dom.getCssNumericValue(
          this.dom.$articleWrapper,
          "borderTopWidth"
        ) -
        this.dom.getCssNumericValue(
          this.dom.$articleWrapper,
      ...
      newArticleInnerWrapperHeight =
        newArticleHeight +
        this.dom.getCssNumericValue(
          this.dom.$articleInnerWrapper,
          "paddingBottom"
        );
      ...
      this.addDomNodeToRecalculate(
        this.dom.$articlePreWrapper,
        newArticleHeight
      );
      this.addDomNodeToRecalculate(
        this.dom.$articleWrapper,
        newArticleWrapperHeight
      );
    ...
    recalculateNavWithTreeOnly: function (tab) {
      ...
      var navOuterPaddingBottom =
        this.dom.getCssNumericValue(
          this.dom.$workZoneSideNavContent,
          "paddingBottom"
        ) + this.getFooterHeight();

      var navInnerPaddingTop = this.dom.getCssNumericValue(
        tab.$wrapperItem,
        "paddingTop"
      );
      var navInnerPaddingBottom = this.dom.getCssNumericValue(
        tab.$wrapperItem,
        "paddingBottom"
      );
      ...
      this.dom.$hasVerticalScrollbar[tabId] =
        realTreeHeight > newNavHeightWithoutHorizontalScrollbar;
      ...
      if (!this.dom.$hasVerticalScrollbar[tabId]) {
        ...
        this.addDomNodeToRecalculate(
          tab.$tree,
          newNavHeightWithoutHorizontalScrollbar - SCROLL_COMPENSATION
        );
      } else {
        //tab.$wrapperItemInner.css( "overflow", "auto" );
        this.addDomNodeToRecalculate(tab.$wrapperItemInner, {
          overflow: "auto",
        });
      }
    ...
    recalculateNavWithTreeAndSearchInput: function (tab) {
      ...
      var navInnerPaddingTop =
        this.dom.getCssNumericValue(tab.$wrapperItem, "paddingTop") +
        this.dom.$tabSearchFormWrapper.height() +
        this.dom.getCssNumericValue(
          this.dom.$tabSearchFormWrapper,
          "marginBottom"
        );
      var navInnerPaddingBottom = this.dom.getCssNumericValue(
        tab.$wrapperItem,
...
DR_EXPLAIN.workZoneSizer_FlowMode = (function () {
  var _class = {
    ...
    recalculate: function () {
      ...
      var articleMustBeResizedBy =
        articleLowestPointMustBe - articleCurrentLowestPoint;

      var articleWrapperHeight = articleWrapper.height();
      articleWrapper.css(
        "min-height",
        Math.max(articleWrapperHeight + articleMustBeResizedBy, 0)
      );
    },
  };
...
DR_EXPLAIN.navTree_Menu = (function () {
  var _class = {
    ...
    init: function () {
      ...
      this.navArr = this.populateTable(
        this.dataManager.getRootNodesArray(),
        true
      );
      var navTreeItemsCollection = this.getNavCollection(this.navArr, null);
      this.navTreeView = navTreeView = new NavTree__View({
        collection: navTreeItemsCollection,
        $navTree: this.$navTree,
      });

      var flatCollection = new NavTree__ItemsNodes_Collection(
        this.navTreeView.models
      );
      flatCollection.on("change:isVisible", this.onNodeVisibleChange, this);
    },
    ...
    renderNode: function (containerElement, node, model, depth) {
      ...
      if (node.isSelected) div.className += " m-tree__itemContent__selected";
      div.title = node.title;

      var emptySpacerCount = depth;
      if (node.childs != null) --emptySpacerCount;
      for (var i = 0; i < emptySpacerCount; ++i)
        div.appendChild(document.createElement("span")).className =
          "b-tree__spacer";
      ...
      if (node.childs != null) {
        ...
        expanderSpan.className = bVisible
          ? "b-tree__i_expander_doClose"
          : "b-tree__i_expander_doOpen";

        var expanderImg = expanderSpan.appendChild(
          document.createElement("span")
        );
        expanderImg.className = bVisible
          ? "expander_img b-tree__i_expander_doClose_inner"
          : "expander_img b-tree__i_expander_doOpen_inner";

        //Add folder icon
        spacer = div.appendChild(document.createElement("span"));
        spacer.className = "b-tree__spacer";
        var folderSpan = spacer.appendChild(document.createElement("span"));
        folderSpan.className = bVisible
          ? "b-tree__i_folder_opened"
          : "b-tree__i_folder_closed";
        var folderSpanInner = folderSpan.appendChild(
          document.createElement("span")
        );
        folderSpanInner.className = bVisible
        ...
        for (var i = 0; i < node.childs.length; ++i)
          this.renderNode(
            ul,
            node.childs[i],
            model.attributes.childs.models[i],
            depth + 1
          );
        node.toggleClasses = function () {
          $(expanderSpan).toggleClass(
            "b-tree__i_expander_doClose b-tree__i_expander_doOpen"
          );
          $(expanderImg).toggleClass(
            "b-tree__i_expander_doClose_inner b-tree__i_expander_doOpen_inner"
          );

          $(folderSpan).toggleClass(
            "b-tree__i_folder_opened b-tree__i_folder_closed"
          );
          $(folderSpanInner).toggleClass(
        ...
        $(expanderSpan).click(function (e) {
          if (bVisible) node.hideExpander();
          else node.showExpander();
        });
        if (!bVisible) {
        ...
        var articleInnerSpan = articleSpan.appendChild(
          document.createElement("span")
        );
      ...
      if (this.navArr !== null) {
        for (var i = 0; i < this.navArr.length; ++i)
          this.renderNode(
            ul,
            this.navArr[i],
            this.navTreeView.collection.models[i],
            1
          );
      }
    ...
    getDefaultNodeVisibleStatusArr: function () {
      ...
      if (
        this.dataManager.getDrexMenuType() == 3 ||
        this.dataManager.getDrexMenuType() == 1
      ) {
        for (var i = 0; i < this.dataManager.getDrex().nodes_count; i++)
          result[i] = 1;
      } else {
        var deep_border =
          this.dataManager.getRootNodesArray().length <= 1 ? 1 : 0;
        for (var i = 0; i < this.dataManager.getDrex().nodes_count; i++) {
          result[i] =
            this.dataManager.getNodeDeepByIndex(i) <= deep_border ? 1 : 0;
        }
      }
      return result;
    },
    ...
    getCurrVisibleStatusArrAsString: function () {
      ...
      for (var i = 0; i < VarTOpnd.length; i++) menuMinimized[i] ^= VarTOpnd[i];
      ...
      for (var i = 0; i < bytes.length; i++)
        if (bytes[i] == 0) bits.push(0);
        else bits.push(1);

      var result = this.utils.bitsToByte(bits, 7);

      for (var i = 0; i < bytes.length; i++)
        if (bytes[i] != 0) result.push(bytes[i]);
    ...
    doSetNodeVisibleStatusArrByUrlEncoder: function () {
      var t = this.urlEncoder.getValueByKey(
        this.urlEncoder.KEY_NAME__MENU_STATE
      );
      ...
      if (t !== null) {
        ...
        for (var i = 0; i < prefixLen * 8; i++)
          if (bits[i] == 0) result.push(0);
          else result.push(bytes[readPos++]);

        var MyBB = this.utils.byteToBits(result, 7);
        for (var i = 0; i < VarTOpnd.length; ++i) VarTOpnd[i] ^= MyBB[i];
      }
    ...
    validateOpenState: function () {
      ...
      var rootNodeIndex = this.dataManager.getIndexByNode(
        this.dataManager.getRootNode()
      );
...
DR_EXPLAIN.navTree_Index = (function () {
  var _class = {
    ...
    init: function () {
      ...
      var navArr = this.populateTable([
        this.dataManager.getDrex().root_keyword(),
      ]);
      var navTreeItemsCollection = this.getNavCollection(navArr, null);
      this.navTreeView = new NavTree__Keywords_View({
        collection: navTreeItemsCollection,
        $navTree: this.$navTree,
      });
    },
    ...
    populateTable: function (keywordArr) {
      ...
      for (var index = 0; index < keywordArr.length; index += 1) {
        ...
        if (keyword.isActive()) {
          ...
          if (childrenArr !== null) itemArr = itemArr.concat(childrenArr);
        }
      }
      return itemArr;
    },
...
DR_EXPLAIN.navTree_Search = (function () {
  var _class = {
    ...
    setNewContentBySearchResults: function (searchResultsArr) {
      ...
      var navTreeItemsCollection = this.getNavCollection(navArr, null);
      var navTreeView = new NavTree__Search_View({
        collection: navTreeItemsCollection,
        $navTree: this.$navTree,
      });
...
function initTabs() {
  ...
  if (app.tabController.tabsCount() == 0) return;

  app.navTree_Menu.init();
  app.navTree_Index.init();
  app.navTree_Search.init();

  if (app.tabController.isMenuTabShown()) app.navTree_Menu.show();
  if (app.tabController.isIndexTabShown()) app.navTree_Index.show();
}
...
function onDocumentReady(app) {
  ...
  if (app.dataManager.getDrexMenuType() === 1) {
    app.dom.tabs.menu.$wrapperItem
      .find(".b-tree")
      .addClass("b-tree__withoutControls");
  }
