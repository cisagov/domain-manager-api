  ...
  var A = (j.each = j.forEach = function (n, t, e) {
    if (null != n)
      if (s && n.forEach === s) n.forEach(t, e);
        ...
        for (var a in n) if (j.has(n, a) && t.call(e, n[a], a, n) === r) return;
  });
    ...
    (j.debounce = function (n, t, r) {
      ...
      return function () {
        var i = this,
          ...
          c = r && !u;
        return (
          clearTimeout(u), (u = setTimeout(o, t)), c && (e = n.apply(i, a)), e
        );
      };
    }),
    (j.once = function (n) {
...
(function () {
  ...
  f.extend(o.prototype, k, {
    ...
    hasChanged: function (a) {
      return !arguments.length
        ? !f.isEmpty(this.changed)
        : f.has(this.changed, a);
    },
  ...
  f.extend(r.prototype, k, {
    ...
    add: function (a, b) {
      ...
      A.apply(
        this.models,
        [null != b.at ? b.at : this.models.length, 0].concat(a)
      );
  ...
  f.extend(m.prototype, k, {
      ...
      this.fragment != c &&
        (this._hasPushState
          ? (0 != c.indexOf(this.options.root) && (c = this.options.root + c),
            (this.fragment = c),
            window.history[b.replace ? "replaceState" : "pushState"](
              {},
              document.title,
              c
            ))
    ...
    _updateHash: function (a, b, c) {
      c
        ? a.replace(a.toString().replace(/(javascript:|#).*$/, "") + "#" + b)
        : (a.hash = b);
    },
  });
...
jQuery.fn.highlight = function (b) {
  function a(e, j) {
    if ($(e).hasClass("drex_highlight")) return 0;
    var l = 0;
    if (e.nodeType == 3) {
      var k = e.data.toUpperCase().indexOf(j);
      if (k >= 0) {
        var h = document.createElement("span");
        h.className = "drex_highlight";
        var f = e.splitText(k);
...
jQuery.fn.removeHighlight = function () {
  return this.find("span.drex_highlight")
    .each(function () {
...
window.NavTree__ItemDecoration_Keyword_View = Backbone.View.extend({
  tagName: "span",
  ...
  html_index_item_folder:
    '<span class="b-tree__i_indexitem"><span class="b-tree__i_indexitem_inner"></span></span>',
  html_index_item_article:
    '<span class="b-tree__i_indexitem"><span class="b-tree__i_indexitem_inner"></span></span>',
...
window.NavTree__ItemDecoration_Search_View = Backbone.View.extend({
  ...
  html_root: '<span class="b-tree__spacer"></span>',
  html_folder_closed:
    '<span class="b-tree__i_folder_closed"><span class="b-tree__i_folder_closed_inner"></span></span>',
  html_article:
    '<span class="b-tree__i_article"><span class="b-tree__i_article_inner"></span></span>',
...
window.NavTree__Item_View = Backbone.View.extend({
  ...
  render: function (childsOutput) {
    var tpl =
      '<div class="b-tree__itemContent" title="<%- title %>"><span class="b-tree__itemText"><a href="<%- link %>" class="b-tree__itemLink"><%- title %></a></span></div>';
    var tplSelected =
      '<div class="b-tree__itemContent m-tree__itemContent__selected" title="<%- title %>"><span class="b-tree__itemText m-tree__itemText__selected"><%- title %></span></div>';
...
window.NavTree__ItemKeyword_View = Backbone.View.extend({
  ...
  render: function (childsOutput) {
    ...
    if (this.model.hasOneLink()) {
      ...
      tpl =
        '<div class="b-tree__itemContent" title="<%- title %>"><span class="b-tree__itemText"><span class="b-tree__itemLink"><%- title %></span></span></div>';
    }
  ...
  bindEvents: function () {
    ...
    if (this.model.hasManyLinks()) {
      this.$el.children("div").on("click", function (e) {
        var currContextMenuVisibleStatus = that.model.get(
      ...
      this.model.on("change:isContextMenuVisible", this.toggleFolder, this);
    }
  },
  ...
  render: function () {
    for (var cid in this.itemsViewArr) {
      ...
      if (itemView.model.hasChilds()) {
        childsOutput = itemView.model.getChildsCollection().itemsView.render()
          .el;
      }
...
window.NavTree__View = Backbone.View.extend({
  ...
  doCreateItemsViewArr: function () {
    if (this.collection) this.fillItemsViewArrWithCollection(this.collection);
  },
  ...
  render: function () {
    ...
    $(this.el).append(
      '<table class="b-tree__layout" cellspacing="0"><tr><td class="b-tree__layoutSide" />'
    );
    if (this.collection)
...
window.NavTree__Keywords_View = NavTree__View.extend({
  ...
  bindContextMenuEvents: function () {
    ...
    $(document).on("click", function () {
      var openedFolderModels = flatCollection.where({
...
window.NavTree__ContextMenu_View = Backbone.View.extend({
  ...
  show: function () {
    ...
    var tpl =
      '<li class="b-tree__item" title="<%- title %>"><div class="b-tree__itemContent"><span class="b-tree__spacer"><span class="b-tree__i_article"><span class="b-tree__i_article_inner"></span></span></span><span class="b-tree__itemText"><a href="<%- link %>" class="b-tree__itemLink"><%- title %></a></span></div></li>';
    this.template = _.template(tpl);
    ...
    var $elemInTree = this.view.$el
      .children("div")
      .children(".b-tree__itemText");
...
DR_EXPLAIN.namespace("DR_EXPLAIN.utils.base64");
DR_EXPLAIN.utils.base64 = {
  encString: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",
  encStringS:
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_",
  encode: function (inp, uc, safe) {
  ...
  decode: function (inp, outType, safe, lax) {
    if (arguments.length < 1) return null;
...
DR_EXPLAIN.namespace("DR_EXPLAIN.utils.utf8");
DR_EXPLAIN.utils.utf8 = {
...
DR_EXPLAIN.namespace("DR_EXPLAIN.utils");
DR_EXPLAIN.utils = {
  base64: DR_EXPLAIN.utils.base64,
  utf8: DR_EXPLAIN.utils.utf8,
  escapeUrl: function (oldUrl) {
  ...
  str2ar: function (str) {
    ...
    for (var i = 0; i < str.length; ++i) retVal.push(str.charCodeAt(i));
    return retVal;
  },
  ar2str: function (arr) {
    var str = "";
  ...
  bitsToByte: function (bits, cnt) {
    var result = new Array();
    while (bits.length % cnt) bits.push(0);
    var MyByte = 0;
  ...
  intDiv: function (nom, denom) {
    nom += denom - 1 - (nom % denom);
    return Math.floor(nom / denom);
  },
  encodingPrefixLen: function (bitsLen, cnt) {
    var bytesLen = this.intDiv(bitsLen + cnt - 1, cnt);
    return this.intDiv(bytesLen + cnt - 1, cnt);
  },
};
...
function InputPlaceholder($input, $label, useNativePlaceholderIfPossible) {
  ...
  if (
    useNativePlaceholderIfPossible !== true &&
...
InputPlaceholder.prototype = {
  ...
  run: function () {
    ...
    if (
      this.useNativePlaceholderIfPossible &&
      this.isNativePlaceholderSupport
    ) {
      this.HideLabel();
  ...
  doBindCustomPlaceholderEvents: function () {
    var that = this;
    this.$input.on(
      "keypress keydown keyup click blur focus paste keypressSync",
      function () {
...
InputSync.prototype = {
  ...
  bindEvents: function () {
    var that = this;
    this.$inputArr.on(
      "keypress keydown keyup click blur focus paste",
      function () {
        that.sync($(this));
      }
    );
...
DR_EXPLAIN.data_menu = {
  ...
  DREX_NODE_NAMES: [
    "Title",
    "Introduction to Domain Manager",
    ...
    "Settings",
    "Glossary",
    "Domain Manager Revision History",
    "Introduction",
    "Overview",
    ...
    "Access",
    "Set up, Launch, and Categorize a Website",
    "Set up a Domain with Email Records",
    "Manage External Websites",
    "Content Authorization",
    "Domain Manager Categorization",
    "Template/Demo",
    "Hosted Zone",
    "DNS Records",
    "Proxy Category",
  ...
  DREX_NODE_LINKS: [
    "index.htm",
    "introduction_to_domain_manager.htm",
    "command_line_interface.htm",
    "domains.htm",
    "templates.htm",
    "applications.htm",
    "users.htm",
    "settings.htm",
    "glossary.htm",
    "domain_manager_revision_history.htm",
    "introduction.htm",
    "overview.htm",
    "domain_manager_workflow.htm",
    "create_account_login.htm",
    "use_cases.htm",
    "admin_vs__user_roles.htm",
    "best_practices.htm",
    "mail_server_setup.htm",
    "cli_requirements_and_setup.htm",
    "commands.htm",
    "upload_a_new_domain.htm",
    "domain_details.htm",
    "upload_a_new_template.htm",
    "template_details.htm",
    "template_resources.htm",
    "add_a_new_application.htm",
    "manage_application.htm",
    "user_details.htm",
    "my_profile.htm",
    "submit_issue.htm",
    "request_feature.htm",
    "help.htm",
    "bugs_and_missing_features.htm",
    "access.htm",
    "set_up__launch__and_categorize_a_website.htm",
    "set_up_a_domain_with_email_records.htm",
    "manage_external_websites.htm",
    "content_authorization.htm",
    "domain_manager_categorization.htm",
    "template_demo.htm",
    "hosted_zone.htm",
    "dns_records.htm",
    "proxy_category.htm",
...
DR_EXPLAIN.data_search = {
  ...
  DREXPLAIN_PREVIEW_MODE_SEARCH_IS_DISABLED_NOTICE:
    "In the preview mode the search is disabled.",
  DREXPLAIN_ERROR_LOCAL_SEARCH:
    "Unable to download search index files. Please, upload HTML files to a web server and access them via URL starting with https:// or http:// .",
  DREXPLAIN_ERROR_REMOTE_SEARCH:
    "Unable to download search index files. Please, ask your web server administrator to enable the downloading of files with .json extension or contact help@drexplain.com. Error code: {0}",
};
